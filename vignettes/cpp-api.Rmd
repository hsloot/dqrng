...
To use the the global RNG, you must include `dqrng_get_rng.h`. The function `get_rng()` returns a non-owning raw pointer of type `dqrng::rng64_t::element_type*`.

```cpp
dqrng::rng64_t::element_type* get_rng()
```

__Warning__: Expert-use only; do not modify (explicitely) or delete the returned pointer (in particular: don't wrap the returned pointer in a smart pointer).
